<?php get_header();
	...
	<section id="primary" class="<?php echo $page_layout;?>">
        <article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
        	<h3 class="border-title "><?php the_title(__('Acerca de: ', 'dt_themes'), '');?><span></span></h3>
            ...
            <div class="column dt-sc-three-fourth space">
				<ul class="teachers-details">
                	<li> <?php echo __('Profesi&oacute;n', 'dt_themes'); ?> : <?php echo dttheme_wp_kses($ts['role']); ?> </li>
                    <li> <?php echo __('Website', 'dt_themes'); ?> : <?php echo '<a href="'.esc_url($ts['url']).'">'.esc_url($ts['url']).'</a>'; ?> </li>
					<li> <?php echo __('Experiencia', 'dt_themes'); ?> : <?php echo dttheme_wp_kses($ts['exp']); ?> </li>
					<li> <?php echo __('Especialista en', 'dt_themes'); ?> : <?php echo dttheme_wp_kses($ts['special']); ?> </li>
				</ul>
				...
				if(is_array($lesson_array) && !empty($lesson_array)) {
					...
					echo '<h5 class="border-title ">'.__('Cursos dictados', 'dt_themes').'<span></span></h5>';
					
					echo '<table class="courses-table-list tablesorter">
							<thead>
							  <tr>
							  	<th class="lessons-table-title-header" scope="col">'.__('Curso','dt_themes').'</th>
								<th class="lessons-table-title-header" scope="col">'.__('Lecci&oacute;n','dt_themes').'</th>
								<th class="lessons-table-type-header" scope="col">'.__('Complejidad','dt_themes').'</th>
								<th class="lessons-table-length-header" scope="col">'.__('Duraci&oacute;n','dt_themes').'</th>
							  </tr>
							</thead>
							<tbody>';
