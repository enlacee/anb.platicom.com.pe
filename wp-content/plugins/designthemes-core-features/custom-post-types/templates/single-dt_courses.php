<?php get_header();
	...
	<section id="primary" class="<?php echo $page_layout;?>">
        ...
        <article id="post-<?php the_ID(); ?>" <?php post_class('dt-sc-course-single'); ?>>
                ...
                <div class="dt-sc-course-details-inner">
                    ...
                    <div class="entry-metadata">
        				<div class="dt-sc-meta-container">
                            ...
                            <?php echo $count.__(' Lecciones', 'dt_themes'); ?>
                            </p>
						...
						if($starting_price != ''): 
							if ( IAMD_USER_ROLE == 's2member_level2' || IAMD_USER_ROLE == 's2member_level3' || IAMD_USER_ROLE == 's2member_level4' || current_user_can($s2_level) ){
								...
								echo '<span class="dt-sc-purchased"> '.__('Adquirido','dt_themes').'</span>';
								...
								$paypal_dec = __('You are about to purchase the Course : ', 'dt_themes').get_the_title($post->ID);
								$paypal_sc = do_shortcode("[s2Member-PayPal-Button level='1' ccaps='cid_{$post->ID}' desc='{$paypal_dec}' ps='paypal' lc='' cc='{$currency}' dg='0' ns='1' custom='".$_SERVER["HTTP_HOST"]."' ta='0' tp='0' tt='D' ra='{$starting_price}' rp='1' rt='L' rr='BN' rrt='' rra='1' image='' output='url'/]");
								
								
							}
						else:
							echo '<span class="dt-sc-purchased"> '.__('Libre','dt_themes').'</span>';
						endif;
						?>
					...
					if(is_user_logged_in() && current_user_can($s2_level)) {
						...
						if($course_status)
							echo '<div class="dt-sc-course-completed"> <span class="fa fa-check-circle"> </span> '.__('Completado', 'dt_themes').'</div>';
					}
					?>
                    </div>
			...
			if(isset($lessons_array) && !empty($lessons_array)) {		
				...
				echo '<div class="dt-lesson-wrapper">
					...
					<h4 class="dt-lesson-title">'.__('Lecciones', 'dt_themes').'<span></span></h4>';
				...
				if(isset($lessons_hierarchy_array[0])) {
					...
					foreach($lessons_hierarchy_array[0] as $lesson) {
						...
						if(isset($lesson_meta_data[0]['lesson-duration']) && $lesson_meta_data[0]['lesson-duration'] != '') {
							$lesson_duration .= '<p> <i class="fa fa-clock-o"> </i>'.dttheme_wp_kses($lesson_meta_data[0]['lesson-duration']). __(' minutos ', 'dt_themes').'</p>';
						}
        ...
        <div class="clear"> </div>
        <div class="dt-sc-hr-invisible"> </div>
        <h3><?php _e('Instructores','dt_themes');?></h3> 
		...
		?>
            ...
            <h3><?php _e('Cursos Relacionados','dt_themes');?></h3> 
            <?php
                    ...
                    <article id="post-<?php echo get_the_ID(); ?>" class="<?php echo implode(" ", get_post_class("dt-sc-custom-course-type", get_the_ID())); ?>">
                        ...
                        <div class="dt-sc-course-thumb">
                            ...
                            <div class="dt-sc-course-overlay">
                                <a title="<?php echo get_the_title(); ?>" href="<?php echo the_permalink(); ?>" class="dt-sc-button small white"> <?php echo __('Ver Curso', 'dt_themes'); ?> </a>
                            </div>
                        </div>
                        ...
                        <div class="dt-sc-course-details">	
                            ...
                            <div class="dt-sc-course-meta">
                                ...
                                <p> <?php echo $count.'&nbsp;'.__('Lecciones', 'dt_themes'); ?> </p>
                            </div>
