           ...
           <div class="column dt-sc-two-third first dt-sc-lessons-content-container">
			<article id="post-<?php the_ID(); ?>" <?php post_class('dt-sc-lesson-single'); ?>>
				  ...
				  <?php
                    ...
                    if(isset($dt_grade_post[0])) {
                        ...
                        if(isset($grade) && $grade != '') {
                            echo '<div class="dt-sc-lesson-completed"> <span class="fa fa-check-circle"> </span> '.__('Completado', 'dt_themes').'</div>';
                        }
				...
				if(isset($lesson_video) && $lesson_video != '') { ?>
                    ...
                    <h4 class="border-title"><?php _e('Video', 'dt_themes'); ?><span></span></h4>
               ...
               <div class="dt-sc-hr-invisible-small"></div>
                ...
                if(isset($dt_lesson_quiz) && $dt_lesson_quiz > 0) {
					...
					if(is_user_logged_in()) {
						...
						if(!isset($dt_grade_post[0])) {
							...
							echo '<input type="submit" name="complete_lesson" value="'.__('Presiona Aqu&iacute; cuando finalices la lecci&oacute;n','dt_themes').'" />';
							echo '</form>';
                ...
                <div class="post-nav-container">
                	<?php if ( $previous_lesson_id > 0 ) { ?><div class="post-prev-link"><a href="<?php echo get_permalink($previous_lesson_id); ?>"><i class="fa fa-arrow-circle-left"> </i> <?php echo ' '.get_the_title($previous_lesson_id).'<span> ('.__('Lecci&oacute;n Anterior','dt_themes').')</span>'; ?></a></div><?php } ?>
                    <?php if (  $next_lesson_id > 0 ) { ?><div class="post-next-link"><a href="<?php echo get_permalink($next_lesson_id); ?>"><?php echo '<span>('.__('Siguiente Lecci&oacute;n','dt_themes').') </span>'.get_the_title($next_lesson_id).' '; ?> <i class="fa fa-arrow-circle-right"> </i></a></div><?php } ?>
                </div><!-- **Post Nav - End** -->
                ...
                <?php
                if(isset($dt_lesson_course) && $dt_lesson_course != '') {
                    echo '<a href="'.get_permalink($dt_lesson_course).'" class="dt-sc-button small alignright">'.__('Volver a ', 'dt_themes').get_the_title($dt_lesson_course).'</a>';	
                }
				?>
            ...
            <div class="column dt-sc-one-third dt-sc-lessons-menu-container">
                ...
                <div class="dt-sc-lessons-menu">
                	...
                	<h4> <?php _e('Maya Curricular del Curso', 'dt_themes'); ?> </h4>
                ...
                <div class="dt-sc-lesson-staff">
                  <div class="lesson-staff-title">
                    <h5><?php echo __('Instructor', 'dt_themes') ?></h5>
                  </div>
                  <div class="lesson-staff-details">
                    ...
                    <div class="team-meta"> 
                        ...
                        <h6> <?php echo __('Profesi&oacute;n', 'dt_themes'); ?> : <?php if(isset($ts['role'])) echo $ts['role']; ?> </h6>
                    </div>
                    <div class="clear"> </div>
                    <ul class="teachers-details">
                        <li> <?php echo __('Website', 'dt_themes'); ?> : <?php if(isset($ts['url'])) echo '<a href="'.$ts['url'].'">'.$ts['url'].'</a>'; ?> </li>
                        <li> <?php echo __('Experiencia', 'dt_themes'); ?> : <?php if(isset($ts['exp'])) echo $ts['exp']; ?> </li>
                        <li> <?php echo __('Especialista en', 'dt_themes'); ?> : <?php if(isset($ts['special'])) echo $ts['special']; ?> </li>
                    </ul>
					<a class="dt-sc-button small" href="<?php echo get_permalink($teacher_data->ID); ?>"><?php echo __('Leer m&aacute;s', 'dt_themes'); ?></a>
                </div>
                </div>
				<?php
				}
