                ...
                <div class="container">
                    ...
                    <div class="header-register">                    	
                        <ul>
                            ...
                            ?>
                                <li><a href="<?php echo $login; ?>" title="<?php _e('Inicia Sesi&oacute;n Ahora', 'dt_themes');?>"><i class="fa fa-user"></i><?php echo __('Inicia Sesi&oacute;n', 'dt_themes'); ?></a></li>
                            <?php
                            else:
                                ...
                                if( dttheme_is_plugin_active('s2member/s2member.php') ):
                                    ...
                                    if( !empty($page) ):
                                        ...
                                        echo '<li><a href="'.$link.'">'.get_avatar( $current_user->ID, 30).'<span>'.__('Bienvenido, ', 'dt_themes').'&nbsp;'.$current_user->display_name.' | </span>'.'</a></li>';
									elseif(!is_null($welcome)):
										echo '<li><a href="'.$welcome.'">'.get_avatar( $current_user->ID, 30).'<span>'.__('Bienvenido, ', 'dt_themes').'&nbsp;'.$current_user->display_name.' | </span>'.'</a></li>';
                                    endif;
                                elseif(!is_null($welcome)):
                                    echo '<li><a href="'.$welcome.'">'.get_avatar( $current_user->ID, 30).'<span>'.__('Bienvenido, ', 'dt_themes').'&nbsp;'.$current_user->display_name.' | </span>'.'</a></li>';
                                else:
                                    echo '<li><a href="#">'.get_avatar( $current_user->ID, 30).'<span>'.__('Bienvenido, ', 'dt_themes').'&nbsp;'.$current_user->display_name.' | </span>'.'</a></li>';
                                endif;	
                                                            
                                echo '<li><a href="'.wp_logout_url(get_permalink()).'" title="'.__('Salir', 'dt_themes').'">'.__('Salir', 'dt_themes').'</a></li>';
                            endif;
                            ?>
                        </ul>
                    </div>
						...
						if( is_page_template('tpl-landingpage.php') ) {
							...
							if (function_exists('wp_nav_menu')) :
								$primaryMenu = wp_nav_menu(array(
											...
											'echo'=>false,
											'container'=>false,
											'items_wrap'      => '<ul id="%1$s" class="group %2$s"><li class="menu-item current-menu-item"><a href="#'.$lp_name.'">'.$lp_title.'</a></li>%3$s</ul>',
